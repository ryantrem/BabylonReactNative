...
//        auto constructorFun = jsiRuntime.global().getPropertyAsFunction(jsiRuntime, "eval").call(jsiRuntime, code);
        ...
        auto code = std::make_shared<const jsi::StringBuffer>("(function(param) { this.testProperty1 = param; })");
        auto testConstructor = jsiRuntime.evaluateJavaScript(std::move(code), "some description that is helpful in a call stack").asObject(jsiRuntime);
        auto testPrototype = testConstructor.getProperty(jsiRuntime, "prototype").asObject(jsiRuntime);
        ...
        jsiRuntime.global().setProperty(jsiRuntime, "TestClass", std::move(testConstructor));
